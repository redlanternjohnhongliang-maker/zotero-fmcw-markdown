...
| ra_d1a_vs_d5c_consistency.csv | 6 | 12 | angle_mask_total_width_bins, bin_mode, candidate, formula, hit_count, hit_criterion, hit_rate, notes |
...
| D5C_RA_RCA_ANALYZE_RESULTS.json | 1 |  | json artifact |
| ra_rca_config.json | 1 |  | json artifact used by analysis |
...
| D5C-RCA recomputed D1A-style | all valid targets / all frames | local RA target patch >= frame p70 and target-background ratio >= 0 | atan2_px_py_current | atan2(px, py) | fftshift_linear_sin | fixed radius 2 bins | 5 | 6168 | 0.8762970168612192 | recomputed in this script as D1A+ path check |
| D5C-RCA recomputed D1A-style | D5C test split / weak targets | local RA target patch >= frame p70 and target-background ratio >= 0 | atan2_px_py_current | atan2(px, py) | fftshift_linear_sin | fixed radius 2 bins | 5 | 262 | 0.683206106870229 | same weak subset but D1A-style criterion |
| D5C-RCA recomputed original fixed-PFA | D5C test split / weak targets | fixed-PFA 2D RA threshold at target PFA 1e-2 | atan2_px_py_current | atan2(px, py) | fftshift_linear_sin | default | 7 | 262 | 0.030534351145038167 | should match D5C prior 0.0305 if metric path is aligned |
| D5C-RCA best fixed-PFA candidate | D5C test split / weak targets | fixed-PFA 2D RA threshold at target PFA 1e-2 | atan2_px_py_unshifted_bins_on_shifted_map | atan2(px, py) | unshifted_bins_on_shifted_map | wide | 10 | 255 | 0.1843137254901961 | best RCA sweep row; smoke-only, not confirmed RA method performance |
