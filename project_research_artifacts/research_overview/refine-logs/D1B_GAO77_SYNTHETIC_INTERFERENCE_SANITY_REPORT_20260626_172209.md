...
|---|---|
| D1B 是否通过 | 未通过 |
| synthetic interference 是否成功注入 | 不充分 |
| achieved SIR 是否接近设定 | 是 |
| range profile 是否出现可见干扰 | 是，图像已保存 |
| background noise floor 是否合理上升 | 不稳定 |
...
| 当前是否建议进入 D2 | 否 |
| 下一步 | 不建议进入 D2；应先修 synthetic interference model 或 fixed-PFA evaluation，使 fixed-PFA weak Pd 退化趋势更稳定。 |
...
不建议进入 D2；应先修 synthetic interference model 或 fixed-PFA evaluation，使 fixed-PFA weak Pd 退化趋势更稳定。
