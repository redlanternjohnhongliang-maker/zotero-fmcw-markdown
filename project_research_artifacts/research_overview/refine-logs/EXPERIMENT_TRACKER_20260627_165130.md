...
| R008B | D5-diagnosis | weak weighting failure diagnosis | balanced_mild vs weak_peak_w2p0; train-size/capacity/weak-definition checks | fixed val/test; effective train samples + small capacity variants | weak Pd gain, hit delta, PFA robustness, overlap contamination | DONE | 不进入 D6；1500 effective samples 下 weak Pd gain=0.0229、+6 weak hits，有训练样本量瓶颈迹象，但使用重复 300 train frames 生成更多 synthetic interference；PFA=1e-3 不支持；capacity gain best=0.0115，wider FCN=0，shallow U-Net 为负；clean_peak weak overlap-mask ratio=0.496，range-only overlap 污染明显 |
| R008C | D5B-D5C | weak definition repair + RD/RA feasibility | repaired weak definitions; RD/RA smoke | fixed val/test; 600-sample sanity; no D6 | weak Pd delta, hit delta, PFA/FA, clean no-harm, mask/non-overlap consistency, RD/RA separability | DONE | `/experiment-audit`=WARN，`/result-to-claim`=partial/warn/medium；repaired definitions 没有达到 weak Pd >=0.02 且 hit delta >=+5，range-only weak weighting 记录为 weak/negative evidence；RD 有 smoke-only feasibility，RA inconclusive；D6 继续 NO-GO |
| R008D | D5D | RD-only constrained supplementary | RD proxy boxes; balanced_mild vs weak_weighting_w2p0 | train-only frozen weak threshold; 2 seeds; no D6 | weak Pd delta, hit delta, PFA/FA, clean no-harm, mask/non-overlap consistency, PFA=1e-3 | DONE | `/analyze-results`=NO-GO，`/experiment-audit`=WARN；mean weak Pd delta=0.0000、mean hit delta=0，PFA/FA 未上升，clean no-harm 通过，但 PFA=1e-3 seed 200 反向；RD 仍是 clean-RD Doppler peak projection proxy，不能写 confirmed RD performance；D6 继续 NO-GO |
| R009 | D6 | false alarm control | weak-target loss + FA penalty | train/val/test | weak Pd@PFA, FAR | TODO | HOLD：D5-check、D5-diagnosis、D5B-D5C 均不支持启动；需先做受限 RD-only feasibility confirmation，不加入 false alarm penalty，再重新 result-to-claim |
