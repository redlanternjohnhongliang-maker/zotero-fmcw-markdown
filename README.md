...
4. 再读 `project_research_artifacts/results/d5i_protocol_data_pivot_decision/D5I_NEXT_STEP_DECISION.md` 和 `D5I_ROUTE_COMPARISON.md`。
5. 然后按问题去 `papers_zotero_markdown/` 查论文，优先看接收端抑制、任务保持、泛化可靠性相关目录。
...
## D5I Protocol/Data Pivot

D5I 是 D5H-Exec no-pass 后的路线决策。最新排序是：

1. `External labeled dataset feasibility`：先用 RADIal、RADDet、CARRADA 等候选数据集锁定真实标签边界；当前只做 feasibility/design，不下载、不训练。
2. `Controlled synthetic RAD protocol sanity`：在 feasibility 表之后，作为下一步最小可执行 unit test，验证 fixed-PFA weak-target metric chain；不能写成真实世界性能。
3. `Negative-result / limitation consolidation`：立即作为 failure ledger 和报告骨架维护。
4. `Gao77 RA calibration RCA`：只作为 diagnostic appendix，不解锁训练或 D6。

下一步推荐执行 D5J：controlled synthetic RAD protocol unit test。仍然禁止训练、D6、weak weighting、false alarm penalty、detector/fixed-PFA 主协议修改和任何 confirmed RD/RA/RAD performance claim。
